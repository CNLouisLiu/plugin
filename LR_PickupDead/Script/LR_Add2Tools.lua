	...
	tWidget = {
		{name = "LR_PickupDead_CheckBox01", type = "CheckBox", text = _L["Enable pickup dead"], x = 0, y = 0, w = 150,
			...
			callback = function(m)
				...
				m[#m + 1] = {bDevide = true}
				m[#m + 1] = {szOption = _L["Book filter settings"]}
				local bookMenu = m[#m]
				bookMenu[#bookMenu + 1] = {szOption = _L["Pickup unread book"], bCheck = true, bMCheck = false, bChecked = function() return LR_PickupDead.UsrData.bPickupUnReadBook end,
					fnAction = function()
						LR_PickupDead.UsrData.bPickupUnReadBook = not LR_PickupDead.UsrData.bPickupUnReadBook
					end,
				}
				bookMenu[#bookMenu + 1] = {szOption = _L["Pickup only one bind book"], bCheck = true, bMCheck = false, bChecked = function() return LR_PickupDead.UsrData.bPickupOnlyOneBindBook end,
					fnAction = function()
						LR_PickupDead.UsrData.bPickupOnlyOneBindBook = not LR_PickupDead.UsrData.bPickupOnlyOneBindBook
					end,
				}
				bookMenu[#bookMenu + 1] = {szOption = _L["Pickup only one not bind book"], bCheck = true, bMCheck = false, bChecked = function() return LR_PickupDead.UsrData.bPickupOnlyOneNotBindBook end,
					fnAction = function()
						LR_PickupDead.UsrData.bPickupOnlyOneNotBindBook = not LR_PickupDead.UsrData.bPickupOnlyOneNotBindBook
					end,
				}
				m[#m + 1] = {bDevide = true}
				m[#m + 1] = {szOption = _L["Give up items beyond the limit of number"], bCheck = true, bMCheck = false, bChecked = function() return LR_PickupDead.UsrData.bGiveUpItemsBTLON end,
					fnAction = function()
						LR_PickupDead.UsrData.bGiveUpItemsBTLON = not LR_PickupDead.UsrData.bGiveUpItemsBTLON
					end,
				}
				--拾取白名单
				m[#m + 1] = {bDevide = true}

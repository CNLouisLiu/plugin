...
function LR_DiamondHelper.GetBagCache()
	...
	local cache = {}
	for dwBox = 1, 6, 1 do
		for dwX = 0, me.GetBoxSize(dwBox) do
			local data = LR_DiamondHelper.GetData(dwBox, dwX)
			if next(data) ~= nil then
...
function LR_DiamondHelper.MoveItemToRecipePosition(v)
	...
	for k, cache in pairs(bagCache) do
		if cache.szName == v.szName and cache.nStackNum >= v.nStackNum and not LR_DiamondHelper.recipe[sformat("%d_%d", cache.dBox, cache.dX)] then

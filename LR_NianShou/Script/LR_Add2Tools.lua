	...
	tWidget = {
		{	name="LR_NianShou_bu01",type="Button",x=0,y=0,text=_L["Start NianShou"], w=200,
			...
			callback = function(m)
				...
				for i=1,16 do
					table.insert (m,{szOption=LR_NianShou_UI_score[i],bCheck=true,bMCheck=true,bChecked=function() return LR_NianShou.UsrData.nUseXiaoJinChui == LR_NianShou_UI_score[i] end,fnAction = function () LR_NianShou.UsrData.nUseXiaoJinChui = LR_NianShou_UI_score[i] end,})
				end
				PopupMenu(m)
			end,
		},{
			...
			callback = function(m)
				local player=GetClientPlayer()
				for i=1,16 do
					table.insert (m,{szOption=LR_NianShou_UI_score[i],bCheck=true,bMCheck=true,bChecked=function() return LR_NianShou.UsrData.nUseJX == LR_NianShou_UI_score[i] end,fnAction = function () LR_NianShou.UsrData.nUseJX = LR_NianShou_UI_score[i] end,})
				end
				PopupMenu(m)
			end,
		},{
			...
			callback = function(m)
				local player=GetClientPlayer()
				for i=1,16 do
					table.insert (m,{szOption=LR_NianShou_UI_score[i],bCheck=true,bMCheck=true,bChecked=function() return LR_NianShou.UsrData.nUseZJ == LR_NianShou_UI_score[i] end,fnAction = function () LR_NianShou.UsrData.nUseZJ = LR_NianShou_UI_score[i] end,})
				end
				PopupMenu(m)
			end,
		},{
			...
			callback = function(m)
				local player=GetClientPlayer()
				for i=1,16 do
					table.insert (m, {szOption = LR_NianShou_UI_score[i], bCheck = true, bMCheck = true, bChecked = function() return LR_NianShou.UsrData.nPausePoint == LR_NianShou_UI_score[i] end,
						fnAction = function ()

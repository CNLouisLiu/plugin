...
function LR_AS_Panel.RClickMenu(realArea, realServer, dwID)
	...
	for groupID, groupV in pairs(LR_AS_Group.GroupList) do
		...
		tinsert (menu2, {
			...
			fnAction = function(UserData)
				...
				local DB = LR.OpenDB(path, "AS_GROUP_CHANGE_GROUP_SAVE_B6931908B2648C2F5FEABFE8816E8257")
	...
	tinsert(menu2, {szOption = _L["Group Cancel"],
		fnAction = function()
			local path = sformat("%s\\%s", SaveDataPath, db_name)
			local DB = LR.OpenDB(path, "AS_GROUP_CANCEL_85E7B24C8750AB7082E646858D6FF2D5")
	...
	tinsert(menu2, {szOption = _L["Add Group"],
		fnAction = function()
			GetUserInput(_L["Group Name"], function(szText)
				...
				if szText ~=  "" then
					local path = sformat("%s\\%s", SaveDataPath, db_name)
					local DB = LR.OpenDB(path, "AS_ADD_GROUP_935E09BF233EC8E405ABB1ED0F639562")
		...
		fnAction = function ()
			local sure_delete = function()
				...
				if next(szKey)~= nil then
					local path = sformat("%s\\%s", SaveDataPath, db_name)
					local DB = LR.OpenDB(path, "MENU_DELETE_PLAYER_810899476BEBD046B845B6F522B0E64F")
